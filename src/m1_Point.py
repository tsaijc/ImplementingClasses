...
class Point(object):
    ...
    def move_to(self, x, y):
        ...
        self.trip = self.trip + math.sqrt(((x - self.x) ** 2) + ((y - self.y) ** 2))
        self.x = x
        self.y = y

    def move_by(self, dx, dy):
        self.moves = self.moves + 1
        self.trip = self.trip + math.sqrt((dx**2) + (dy**2))
        self.x = self.x + dx
        self.y = self.y + dy
    ...
    def get_distance_from_start(self):
        return math.sqrt(((self.initX - self.x) ** 2) + ((self.initY - self.y) ** 2))
    ...
    def halfway_to(self, p2):
        return Point((p2.x - self.x)/2 + self.x,(p2.y - self.y)/2 + self.y)
